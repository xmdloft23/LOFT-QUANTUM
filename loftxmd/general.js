const { gmd, commands, monospace, formatBytes } = require("../loft"),
  fs = require("fs"),
  axios = require("axios"),
...
╰─────────────⦁`;
      ...
      await Loftxmd.sendMessage(from, LoftxmdMess, { quoted: mek });
      await react("✅");
    } catch (e) {
      console.error(e);
      reply(`${e}`);
...
╰─────────────╯${readmore}\n`;
      ...
      await Loftxmd.sendMessage(from, loftMess, { quoted: mek });
      await react("✅");
    } catch (e) {
      console.error(e);
      reply(`${e}`);
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    const response = await axios.get(
      `https://api.github.com/repos/${loftRepo}`,
    );
      ...
      buttons: [
        {
          ...
          buttonParamsJson: JSON.stringify({
            ...
            copy_code: `https://github.com/${loftRepo}`,
          }),
        },
        {
          name: "cta_url",
    ...
    const handleResponse = async (event) => {
      ...
      try {
        const zipUrl = `https://github.com/${loftRepo}/archive/refs/heads/main.zip`;
        await Loftxmd.sendMessage(
          from,
          {
            document: { url: zipUrl },
        ...
        await Loftxmd.sendMessage(from, { text: "Failed to download repo zip: " + dlErr.message }, { quoted: messageData });
      }
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      await Loftxmd.sendMessage(sender, mediaData, { quoted: mek });
      await react("✅");
    } catch (error) {
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    const { q, reply, react, botFooter, botPrefix, LoftxmdTechApi, LoftxmdApiKey } = conText;
    ...
    try {
      const meta = await Loftxmd.newsletterMetadata("invite", inviteKey);
      ...
      await sendButtons(Loftxmd, from, sendOpts);
      await react("✅");
    } catch (error) {
