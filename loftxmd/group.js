const { gmd, getGroupMetadata, getLidMapping } = require("../loft");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      const gInfo = await getGroupMetadata(Loftxmd, from);
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    const convertLidToJid = async (lid) => {
      ...
      try {
        const result = await Loftxmd.getJidFromLid(lid);
    ...
    const isTargetSuperUser = await isSuperUser(targetJid, Loftxmd);
    ...
    try {
      await Loftxmd.groupParticipantsUpdate(from, [targetJid], "demote");
      await react("✅");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      await Loftxmd.groupParticipantsUpdate(from, [targetJid], "promote");
      await react("✅");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      await Loftxmd.groupParticipantsUpdate(from, [targetJid], "remove");
      await react("✅");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      const result = await Loftxmd.groupParticipantsUpdate(
        from,
        [targetJid],
        "add",
      );
      ...
      if (status === "403") {
        const meta = await Loftxmd.groupMetadata(from);
        const groupName = meta.subject;
        const inviteCode = await Loftxmd.groupInviteCode(from);
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      const group = await Loftxmd.groupCreate(groupName, [sender]);

      const inviteCode = await Loftxmd.groupInviteCode(group.id);
      const inviteLink = `https://chat.whatsapp.com/${inviteCode}`;
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      if (membersToRemove.length > 0) {
        await Loftxmd.groupParticipantsUpdate(from, membersToRemove, "remove");
      }

      await Loftxmd.groupLeave(from);
    } catch (error) {
      await react("❌");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      await Loftxmd.groupRequestParticipantsUpdate(from, [userJid], "approve");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      await Loftxmd.groupRequestParticipantsUpdate(from, [userJid], "reject");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      const pendingRequests = await Loftxmd.groupRequestParticipantsList(from);
      ...
      await Loftxmd.groupRequestParticipantsUpdate(from, jids, "approve");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      const jids = pendingRequests.map((r) => r.jid);
      await Loftxmd.groupRequestParticipantsUpdate(from, jids, "reject");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      const groupMeta = await Loftxmd.groupMetadata(from);
      ...
      Loftxmd.ev.on("presence.update", presenceHandler);
      ...
      try {
        ...
        for (let i = 0; i < participants.length; i += batchSize) {
          ...
          await Promise.all(
            batch.map(async (p) => {
              ...
              try {
                await Loftxmd.presenceSubscribe(jid);
              } catch (e) {}
            }),
          );
        ...
        Loftxmd.ev.off("presence.update", presenceHandler);
      }
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      await Loftxmd.groupRevokeInvite(from);

      const newInviteCode = await Loftxmd.groupInviteCode(from);
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      await Loftxmd.loftStatus.sendGroupStatus(from, statusPayload);
      await react("✅");
    } catch (error) {
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      await Loftxmd.groupUpdateSubject(from, q);
      await react("✅");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      await Loftxmd.groupUpdateDescription(from, q);
      await react("✅");
