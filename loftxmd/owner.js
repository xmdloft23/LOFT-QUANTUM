...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      tempFilePath = await Loftxmd.downloadAndSaveMediaMessage(
        quotedImg,
        "temp_media",
      );
    ...
    try {
      if (quoted) {
        if (isGroup && !targetUser.endsWith("@s.whatsapp.net")) {
          try {
            const jid = await Loftxmd.getJidFromLid(targetUser);
            if (jid) targetUser = jid;
          } catch (error) {}
        }

        try {
          profilePictureUrl = await Loftxmd.profilePictureUrl(
            targetUser,
            "image",
          );
        } catch (error) {
        ...
        try {
          const statusData = await Loftxmd.fetchStatus(targetUser);
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      const tempFilePath = await Loftxmd.downloadAndSaveMediaMessage(
        quotedImg,
        "temp_media",
      );
      const imageBuffer = await fs.readFile(tempFilePath);
      try {
        await Loftxmd.updateProfilePicture(Loftxmd.user.id, {
          url: tempFilePath,
        });
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      let profilePictureUrl;
      try {
        profilePictureUrl = await Loftxmd.profilePictureUrl(from, "image");
      } catch (error) {
        await react("❌");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      await Loftxmd.sendMessage(from, msg);
      await react("✅");
    } catch (e) {
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      await Loftxmd.sendMessage(sender, msg);
      await react("✅");
    } catch (e) {
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      await Loftxmd.sendMessage(from, { disappearingMessagesInChat: duration });
  ...
  async (from, Loftxmd, conText) => {
   ...
   // if (!isGroup) return reply("❌ This command only works in groups!");
    ...
    try {
      ...
      await Loftxmd.sendMessage(from, { delete: quotedKey });
      if (mek?.key) {
        await Loftxmd.sendMessage(from, { delete: mek.key });
      }
      await react("✅");
    } catch (error) {
      await react("❌");
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
        ...
        await Loftxmd.sendMessage(from, { text: message });
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      await Loftxmd.updateBlockStatus(targetJid, "block");
      await react("✅");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      await Loftxmd.updateBlockStatus(targetJid, "unblock");
      await react("✅");
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      const blockedList = await Loftxmd.fetchBlocklist();
    ...
    try {
      ...
      if (msgType === "conversation" || msgType === "extendedTextMessage") {
        ...
        try {
          ...
          buffer = await altDownload(fakeMsg, Loftxmd);
        }
        ...
        if (msgType === "imageMessage") {
          ...
          await Loftxmd.sendMessage(targetJid, { sticker: buffer });
        }
      } else {
    ...
    try {
      ...
      if (msgType === "conversation" || msgType === "extendedTextMessage") {
        ...
        await Loftxmd.sendMessage(
          ...
          { statusJidList: await getStatusJidList(Loftxmd) },
        );
        ...
        const buffer = await downloadMediaMessage(fakeMsg, Loftxmd);
        if (!buffer) {
          return reply("❌ Failed to download media!");
        }
        ...
        const caption =
          customCaption !== null ? customCaption : originalCaption;
        const statusJidList = await getStatusJidList(Loftxmd);
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      const groupId = await Loftxmd.groupAcceptInvite(inviteCode);
...
async function getStatusJidList(Loftxmd) {
  try {
    const contacts = await Loftxmd.groupFetchAllParticipating();
    const jidList = [];
  ...
  async (from, Loftxmd, conText) => {
    ...
    if (q && q.trim()) {
      ...
      if (quotedUser.endsWith("@lid")) {
        try {
          const jid = await Loftxmd.getJidFromLid(quotedUser);
    ...
    try {
      const [result] = await Loftxmd.onWhatsApp(targetNumber);
...
gmd(
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      setTimeout(() => Loftxmd.ev.off("messages.upsert", handleResponse), 5 * 60 * 1000);
    ...
    try {
      ...
      if (input) {
        ...
        if (groupLinkMatch) {
          ...
          const meta = await Loftxmd.groupGetInviteInfo(code);
          ...
          const meta = await Loftxmd.newsletterMetadata("invite", key);
          finalResult = meta?.id || null;
          label = "Channel JID";
        ...
        if (result.endsWith("@lid")) {
          ...
          if (cached) {
            ...
            try {
              const resolved = await Loftxmd.getJidFromLid(result);
  ...
  async (from, Loftxmd, conText) => {
    ...
    try {
      ...
      if (target.startsWith("@") && !target.includes("@lid")) {
        ...
        try {
          const lid = await Loftxmd.getLidFromJid(target);
          if (lid) {
            target = lid;
