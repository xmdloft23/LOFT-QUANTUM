const { gmd, toAudio, toVideo, toPtt, stickerToImage, gmdFancy, gmdRandom, getSetting, runFFmpeg, getVideoDuration, gmdSticker } = require("../loft");
    ...
    try {
        ...
        try {
            if (quotedImg || quotedVideo) {
                ...
                await fs.unlink(mediaFile).catch(() => {});
                await react("✅");
                return Loftxmd.sendMessage(from, { sticker: stickerBuffer }, { quoted: mek });

            } else if (quotedSticker) {
                // Sticker → Sticker (recompress if too big)
                tempFilePath = await Loftxmd.downloadAndSaveMediaMessage(quotedSticker, "temp_media");
                ...
                await react("✅");
                return Loftxmd.sendMessage(from, { sticker: newStickerBuffer }, { quoted: mek });
            }
        } finally {
...
gmd({
  ...
  async (from, Loftxmd, conText) => {
    ...
    let tempFilePath;
    try {
      tempFilePath = await Loftxmd.downloadAndSaveMediaMessage(quotedVideo, 'temp_media');
...
gmd({
  ...
  async (from, Loftxmd, conText) => {
    ...
    let tempFilePath;
    try {
      tempFilePath = await Loftxmd.downloadAndSaveMediaMessage(quotedAudio, 'temp_media');
      const buffer = await fs.readFile(tempFilePath);
