...
Baada ya kulipa:
• Tumia screenshot hapa
• Au reply: PAID [Order ID] [M-Pesa ID]
...
⏳ Inasubiri malipo...`;
        ...
        // ─── 2. Maelekezo ya Malipo (simple text) ──────────────────
        const paymentInfo = `╭══════════════════════════════╮
║     JINSI YA KULIPA          ║
╰══════════════════════════════╯
...
Kiasi cha kulipa : TSh ${formatNumber(total)}
Order ID         : ${orderId}

Tumia M-Pesa au Card kwa namba hapo juu.

Baada ya kulipa:
1. Tumia screenshot hapa
2. Au reply: PAID ${orderId} [M-Pesa ID]
3. Au mjumuishe moja kwa moja kwa ${SELLER_NUMBER}`;
        ...
        // ─── 3. Ujumbe wa mwisho + Audio (optional) ───────────────
        await sock.sendMessage(chatId, {
            text: `✅ *Order ${orderId} Imepokewa!*\n\nTafadhali endelea na malipo ili tuwasilishe scripts zako haraka.\n\nAsante kwa kutuamini! 🚀`
        }, { quoted: message });
...
Muda       : ${new Date().toLocaleString('sw-TZ')}`;
        ...
        const reply = err.message?.includes('timeout') || err.message?.includes('network')
            ? '⚠️ Tatizo la mtandao. Jaribu tena baadaye.'
            : '⚠️ Hitilafu imetokea. Jaribu tena kidogo.';
