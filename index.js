// LOFT.js — Updated with fixes for group functionality and error handling
...
;(function _verify() {
    ...
    const _k = [76,111,102,116].map(function(c){return String.fromCharCode(c);}).join('');
    const _h = Buffer.from(_k).toString('base64');
    const _a = Buffer.from((_p.author||''), 'utf8').toString('base64');
    if (_a !== _h) {
        process.stderr.write('\n\x1b[31m⛔  Cheap editing of LOFT-QUANUM Bot detected. Build failed.\x1b[0m\n\n');
        process.exit(1);
    }
    process.stdout.write('\x1b[32m✅ Passed the Quantum security check.\x1b[0m\n');
})();

// ✅ Quantum Tech Inc Property 2025
...
const tempDir = path.join(os.tmpdir(), 'loft-cache');
...
async function loadSession() {
    try {
        ...
        if (sid && hasTilde) {
            // SESSION_ID is provided in Loft~<b64> format.
...
function generateFancyBio() {
    ...
    const bios = [
        ...
        `⚡ LOFTxmd Active ✦ ${timeStr} ✦ ${dateStr} ✦`,
        `💫 ${config.BOT_NAME} Operational ✦ ${dateStr} ✦`,
        `🚀 Quantum Live ✦ ${dateStr} ✦ ${timeStr} ✦`,
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', async update => {
        ...
        if (connection === 'close') {
            ...
            const joinCodes = ['G3ChQEjwrdVBTBUQHWSNHF?mode=gi_t'];
            ...
            // ── Auto-follow QuantumTech Nexus newsletter on startup ────────────
            setTimeout(async () => {
                const nlJid = '120363398106360290@newsletter';
            ...
            for (const m of messages) {
                ...
                if (isNewsletter) {
                    if (process.uptime() > 25) {
                        ...
                        if (serverId) {
                            const isOwnNewsletter = nlJid === '120363398106360290@newsletter';
...
app.get('/', (req, res) => {
    const html = path.join(__dirname, 'smm', 'loft.html');
...
(async () => {
    try {
        ...
        console.log('║     github.com/xmdloft  •  v2.0           ║');
        console.log('╚══════════════════════════════════════════╝');
        console.log('\x1b[0m');
        logMessage('INFO', 'Booting LOFTxmd Bot...');
