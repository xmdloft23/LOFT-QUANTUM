...
const REPO    = 'https://github.com/xmdloft23/loft-quantum';
const WEBSITE = 'https://quantum-three-taupe.vercel.app/';
const TZ      = 'Africa/Nairobi';

// ── Category definitions ─────────────────────────────────────────────────────
const CATEGORIES = [
    { icon: '🌐',  name: 'Downloaders',        cmds: ['yt','tiktok','instagram','facebook','apk','catbox'] },
    ...
    { icon: '📺', name: 'Media & Stickers',    cmds: ['sticker','vv','ascii','qrcode','react'] },
    ...
    { icon: '📡', name: 'Bot Info',            cmds: ['alive','ping','uptime','owner','getjid','repo','remind'] },
];
...
function buildMenuText(plugins, pfx, botName, ownerNum, mode) {
    ...
    const footer = [
        ...
        `╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄╯`,
        ``,
        `> ⚡ _${getStr('by') || 'Powerd By LOFTxmd'} © ${now.year()}_`
    ].join('\n');
...
async function sendCallLogMenu(sock, jid, menuText, imgUrl) {
    ...
    const quotedCtx = {
        ...
        externalAdReply: {
            title:                 `${getStr('botName') || 'LOFT-QUANTUM'} — Command Menu`,
...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        const botName  = getStr('botName') || config.BOT_NAME || 'LOFT-QUANTUM';
        const ownerNum = `+${(config.OWNER_NUMBER || '').replace(/\D/g, '')}`;
        const mode     = (config.MODE || 'public').toUpperCase();
        const pfx      = prefix || '.';
        const imgUrl   = getStr('pic1') || config.ALIVE_IMG || 'https://files.catbox.moe/d4nl2o.jpg';
