...
async function LoftxmdAutoReact(emoji, ms,Loftxmd) {
  try {
    const react = {
      react: {
        text: emoji,
    ...
    await Loftxmd.sendMessage(ms.key.remoteJid, react);
  } catch (error) {
...
const LoftxmdAntiLink = async (Loftxmd, message, getGroupMetadata) => {
    try {
        ...
        if (sender.endsWith('@lid')) {
            ...
            if (cached) {
                ...
                try {
                    const resolved = await Loftxmd.getJidFromLid(sender);
        ...
        try {
            await Loftxmd.sendMessage(from, { delete: message.key });
        } catch (delErr) {
        ...
        if (action === 'kick') {
            try {
                await Loftxmd.groupParticipantsUpdate(from, [sender], 'remove');
                await Loftxmd.sendMessage(from, {
...
const LoftxmdAntibad = async (Loftxmd, message, getGroupMetadata) => {
    try {
        if (!message?.message || message.key.fromMe) return;
        const from = message.key.remoteJid;
        const isGroup = from.endsWith('@g.us');
...
const LoftxmdAntiGroupMention = async (Loftxmd, message, getGroupMetadata) => {
    try {
        ...
        if (sender.endsWith('@lid')) {
            ...
            if (cached) {
                ...
                try {
                    const jidResult = await Loftxmd.getJidFromLid(sender);
        ...
        const groupMetadata = await getGroupMetadata(Loftxmd, groupJid);
        if (!groupMetadata || !groupMetadata.participants) return;
        ...
        if (action === 'delete') {
            try {
                await Loftxmd.sendMessage(groupJid, { delete: message.key });
            ...
            try {
                await Loftxmd.groupParticipantsUpdate(groupJid, [sender], 'remove');
                await Loftxmd.sendMessage(groupJid, {
...
const LoftxmdAutoBio = async (Loftxmd) => {
                try {
                    const settings = await getAllSettings();
                    const botName = settings.BOT_NAME || '𝐀𝐓𝐀𝐒𝐒𝐀-𝐌𝐃';
                    ...
                    await Loftxmd.updateProfileStatus(bioText);
                } catch (error) {
                }
            };
...
async function getAIResponse(query) {
    if (isIdentityQuestion(query)) {
        return 'I am an Interactive Ai Assistant Chat Bot, created by Loftxmd Tech!';
    }
...
function LoftxmdChatBot(Loftxmd, chatBot, chatBotMode, createContext, createContext2, googleTTS) {
...
const LoftxmdPresence = async (Loftxmd, jid) => {
    try {
        const isGroup = jid.endsWith('@g.us');
        ...
        await Loftxmd.sendPresenceUpdate(whatsappPresence, jid);
...
const LoftxmdAnticall = async (json, Loftxmd) => {
   const settings = await getAllSettings();
   ...
   for (const id of json) {
      if (id.status === 'offer') {
         if (antiCall === "true" || antiCall === "decline") {
            let msg = await Loftxmd.sendMessage(id.from, {
               text: `${antiCallMsg}`,
               mentions: [id.from],
            });
            await Loftxmd.rejectCall(id.id, id.from);
            ...
            await Loftxmd.rejectCall(id.id, id.from); 
            await Loftxmd.updateBlockStatus(id.from, "block");
         }
      }
   }
};
...
const LoftxmdAntiDelete = async (Loftxmd, deletedMsg, key, deleter, sender, botOwnerJid, deleterPushName, senderPushName) => {
    const settings = await getAllSettings();
    const botName = settings.BOT_NAME || '𝐀𝐓𝐀𝐒𝐒𝐀-𝐌𝐃';
    ...
    const resolveLidToJidAndDisplay = async (lid, pushName, groupJid) => {
        ...
        if (lid.endsWith('@lid')) {
            ...
            if (!jid && Loftxmd.getJidFromLid) {
                try {
                    jid = await Loftxmd.getJidFromLid(lid);
                } catch (e) {}
            }
            
            if (!jid && groupJid && isJidGroup(groupJid)) {
                try {
                    const groupMeta = await getGroupMetadata(Loftxmd, groupJid);
    ...
    if (isJidGroup(key.remoteJid)) {
        try {
            const groupMeta = await getGroupMetadata(Loftxmd, key.remoteJid);
            ...
            promises.push((async () => {
                try {
                    ...
                    if (deletedMsg.message?.conversation || deletedMsg.message?.extendedTextMessage?.text) {
                        ...
                        await Loftxmd.sendMessage(key.remoteJid, {
                            text: `${baseAlert}\n\n📝 *Content:* ${text}`,
            ...
            promises.push((async () => {
                try {
                    ...
                    if (deletedMsg.message?.conversation || deletedMsg.message?.extendedTextMessage?.text) {
                        ...
                        await Loftxmd.sendMessage(botOwnerJid, { 
...
const LoftxmdAntiViewOnce = async (Loftxmd, message) => {
    try {
        if (!message?.message) return;
        if (message.key.fromMe) return;
        ...
        try {
            tempFilePath = await Loftxmd.downloadAndSaveMediaMessage(mediaMessage, path.join(tempDir, tempFileName));
            ...
            if (sendContent) {
                await Loftxmd.sendMessage(targetJid, sendContent);
            }
        } catch (e) {
...
const _resolveLid = async (Loftxmd, lid) => {
    if (!lid?.endsWith('@lid')) return lid;
    const { getLidMapping } = require('./connection/groupCache');
    const cached = getLidMapping(lid);
    if (cached) return cached;
    try { const r = await Loftxmd.getJidFromLid(lid); if (r) return r; } catch (e) {}
    return lid;
};

const LoftxmdAntiEdit = async (Loftxmd, updateData, findOriginal) => {
    try {
        const settings = await getAllSettings();
        ...
        const resolvedChatJid = await _resolveLid(Loftxmd, rawChatJid);
        ...
        sender = await _resolveLid(Loftxmd, sender);
        ...
        if (isGroup) {
            try { const meta = await getGroupMetadata(Loftxmd, resolvedChatJid); chatLabel = meta?.subject || resolvedChatJid; } catch (e) {}
        }
            ...
            if (originalMediaObj) {
                try {
                    ...
                    if (origMsgType === 'imageMessage') {
                        await Loftxmd.sendMessage(targetJid, { image: buffer, caption: alertText, mentions });
                    } else if (origMsgType === 'videoMessage') {
                        await Loftxmd.sendMessage(targetJid, { video: buffer, caption: alertText, mentions });
                    } else if (origMsgType === 'documentMessage') {
                        await Loftxmd.sendMessage(targetJid, {
                            document: buffer,
                        ...
                        await Loftxmd.sendMessage(targetJid, { text: alertText, mentions });
                    }
                    return;
                } catch (mediaErr) {
        ...
        const dmTarget = Loftxmd.user?.id ? `${Gifted.user.id.split(':')[0]}@s.whatsapp.net` : null;
...
module.exports = { logger, emojis, LoftxmdAutoReact, GiftedTechApi, GiftedApiKey, LoftxmdAntiLink, LoftxmdAntibad, LoftxmdAntiGroupMention, LoftxmdAutoBio, LoftxmdChatBot, LoftxmdAntiDelete, LoftxmdAnticall, LoftxmdPresence, LoftxmdAntiViewOnce, LoftxmdAntiEdit };
