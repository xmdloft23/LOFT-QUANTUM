...
const serializeMessage = async (ms, Loftxmd, settings = {}) => {
    ...
    const sendr = ms.key.fromMe 
        ? (Loftxmd.user.id.split(':')[0] + '@s.whatsapp.net' || Loftxmd.user.id) 
    ...
    return {
        ...
        args,
        q: args.join(' '),
        pushName: ms.pushName || (ms.key.fromMe ? Loftxmd.user?.name : null) || 'LOFT-QUANTUM User',
        quoted,
        repliedMessage,
        mentionedJid,
        tagged,
